...
from src.facade.magento.magento_facade import MagentoConnection
...
class InternalAutomationFacade:
    ...
    def connectFTPServer(self) -> FTPFacade:
        ...
        port = 21
        return FTPFacade(host, port, username, password)

    def connectFishbowl(self) -> FishbowlFacade:
        ...
        return FishbowlFacade(
            config["Username"], password, config["Host"],
            config["Port"]
        )

    def connectOutlook(self) -> OutlookFacade:
        ...
        return OutlookFacade(config, password, consumerSecret)
    ...
    def buildSOString(self, customer: str, order: Order) -> str:
        ...
        string += rf'"{order.address.vendor_name}", "{order.address.street}", '
