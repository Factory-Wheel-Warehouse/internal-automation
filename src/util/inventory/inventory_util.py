...
from src.util.inventory.pricing_util import get_marked_up_price
...
def _get_price(price_map: dict, part_num: str, cost: float) -> float:
    stored_price = price_map.get(part_num)
    if stored_price:
        return stored_price
    else:
        # TODO: Add cost adjustment
        if cost > 0 and re.match(FINISH_PATTERN, part_num):
            price = get_marked_up_price(cost)
            price_map[part_num] = price
            return price
    return -1
...
def add_vendor_inventory(ftp: FTPFacade, inventory: dict,
    ...
    for row in _get_file(ftp, vendor.inventory_file_config):
        ...
        if part_num:
            ...
            price = _get_price(price_map, part_num, cost)
